...
echo "     ██   ██ ██       ██ ██  ██  ██ ██ ██    ██ ██   ██ ██           ██"; 
echo "    ██   ██ ███████ ██   ██ ██   ████  ██████  ██████  ███████ ███████";
echo "   Cosmovisor Automatic Installer for Dhealth | Chain ID : dhealth ";
echo -e "\e[0m"
...
REPO=https://ss.dhealth.nodestake.org/dhealthd
GENESIS=https://ss.dhealth.nodestake.org/genesis.json
ADDRBOOK=https://ss.dhealth.nodestake.org/addrbook.json
PORT=121
...
# Set peers and seeds
SEEDS="3fed663a0fd0b7d57d6f5ee128d527f4a07435aa@rpc.dhealth.nodestake.org:666"
PEERS="$(curl -sS https://rpc.dhealth.nodestake.org/net_info | jq -r '.result.peers[] | "\(.node_info.id)@\(.remote_ip):\(.node_info.listen_addr)"' | awk -F ':' '{print $1":"$(NF)}' | sed -z 's|\n|,|g;s|.$||')"
...
curl -o - -L https://ss.dhealth.nodestake.org/${SNAP_NAME}  | lz4 -c -d - | tar -x -C $HOME/$FOLDER
