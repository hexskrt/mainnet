...
# Get mainnet version of Composable Centauri
cd $HOME
rm -rf $SOURCE
git clone $REPO
cd $SOURCE
...
# Set peers and seeds
SEEDS="400f3d9e30b69e78a7fb891f60d76fa3c73f0ecc@composable.rpc.kjnodes.com:15959"
PEERS="$(curl -sS https://composable.rpc.kjnodes.com/net_info | jq -r '.result.peers[] | "\(.node_info.id)@\(.remote_ip):\(.node_info.listen_addr)"' | awk -F ':' '{print $1":"$(NF)}' | sed -z 's|\n|,|g;s|.$||')"
...
curl -L https://snapshots.kjnodes.com/composable/snapshot_latest.tar.lz4 | tar -Ilz4 -xf - -C $HOME/$FOLDER
