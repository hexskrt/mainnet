...
echo "     ██   ██ ██       ██ ██  ██  ██ ██ ██    ██ ██   ██ ██           ██"; 
echo "    ██   ██ ███████ ██   ██ ██   ████  ██████  ██████  ███████ ███████";
echo "    Cosmovisor Automatic Installer for Sourcec | Chain ID : source-1";
echo -e "\e[0m"
...
GENESIS=https://snap.hexnodes.co/source/genesis.json
ADDRBOOK=https://snap.hexnodes.co/source/addrbook.json
PORT=115
...
# Set peers and seeds
SEEDS="eb81c7a9980840f7c9c3631dc23db48e2fae5c94@95.214.54.251:13656"
PEERS="$(curl -sS https://rpc.source.hexnodes.co/net_info | jq -r '.result.peers[] | "\(.node_info.id)@\(.remote_ip):\(.node_info.listen_addr)"' | awk -F ':' '{print $1":"$(NF)}' | sed -z 's|\n|,|g;s|.$||')"
...
curl -L https://snap.nodex.one/source/source-latest.tar.lz4 | tar -Ilz4 -xf - -C $HOME/$FOLDER
